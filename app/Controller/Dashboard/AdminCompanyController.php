...
class AdminCompanyController extends AppController
{
    ...
    private function importLogo($id){
        ...
        if($handle->uploaded){
            ...
            $handle->file_new_name_body = "pe".rand(3,999999999);
            ...
            if($handle->processed){
                $updateLogo = $this->Company->update(
                    'id', (int) $id,
                    [
                        'logo' => $handle->file_dst_name_body. '.' . $handle->file_dst_name_ext
                    ]
                );
                if($updateLogo){
    ...
    private function importSignature($id){
        $handle = new Upload($_FILES['signature'], 'fr_FR');
        if($handle->uploaded){
            $handle->allowed = array('image/*');
            $handle->process(ROOT . 'public/uploads/company/');
            if($handle->processed){
                $updateLogo = $this->Company->update(
                    'id', (int) $id,
                    [
                        'your_signature' => $handle->file_src_name
                    ]
                );
                if($updateLogo){
                    $this->session->setFlash('success', 'La nouvelle signature a bien été chargée');
                }
            } else {
                $this->session->setFlash('danger', 'Un problème est survenu -> ' . $handle->error);
            }
