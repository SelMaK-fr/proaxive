...
class AdminEquipmentController extends AppController
{
    ...
    {
        $e = $this->Equipment->findWithClientCategoryBrandsOS($id);
        $i = $this->Intervention->allById('equipment_id', (int) $id);
        //Breadcrumb
        $this->breadcrumbs->addCrumb('Accueil', '/admin')
        ...
        $this->render('equipments/admin/show.twig', [
            'e' => $e,
            'interventions' => $i,
            'current_menu' => $this->current_menu,
            'breadcrumbs' => $this->breadcrumbs
    ...
    public function edit(int $id){
        if(!empty($_POST)){
            $result = $this->Equipment->update(
                ...
                [
                    ...
                    'u_account_mail' => $_POST['u_account_mail'],
                    'u_password' => $_POST['u_password'],
                    'u_domain' => $_POST['u_domain'],
                    'n_ipaddress' => $_POST['n_ipaddress'],
                    'n_gateway' => $_POST['n_gateway'],
                    'n_masknetwork' => $_POST['n_masknetwork'],
                    'n_dns' => $_POST['n_dns'],
                    'n_ssid' => $_POST['n_ssid'],
                    'n_wifi_key' => $_POST['n_wifi_key'],
                    //
                    'client_id' => $_POST['client_id'],
                    'brand_id' => $_POST['brand_id'],
                    'category_equipment_id' => $_POST['category_equipment_id'],
        ...
        if($equipment === false) {
            $this->session->setFlash('danger', "Cet équipement n'existe pas !");
            return $this->redirect('/admin/equipments');
        }
    ...
    public function exportEquipment(){
        ...
        if(!empty($_POST)){
            $exportData = new CSV();
            $exportData->export_thead($equipments, 'ProaxiveLite_Export_Equipments-'.date('Y-m-d-H_i_s'));
        }
    }
