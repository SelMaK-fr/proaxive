...
class AdminInterventionController extends AppController{
    ...
    public function __construct(){
        ...
        $this->loadModel('InterventionTask');
    ...
    public function home(){
        ...
        $selectIsNOtApproved = $this->Intervention->countIsNotApproved();
        ...
        $this->render('intervention/admin/home.twig', [
            ...
            'selectIsNOtApproved' => $selectIsNOtApproved,
            'navTabs' => 'approved'
        ]);
    ...
    {
        $interventions = $this->Intervention->allIsNotApproved();

        //Breadcrumb
        $this->breadcrumbs->addCrumb('Accueil', '/admin')
            ->addCrumb('Interventions', '/admin/interventions')
            ->addCrumb('A approuver', '');
        $this->breadcrumbs->render();
        ...
        $this->render('intervention/admin/home.twig', [
            ...
            'current_menu' => $this->current_menu,
            'breadcrumbs' => $this->breadcrumbs,
            'selectIsNOtApproved' => $interventions,
            'navTabs' => 'notApproved'
        ]);
    }
        ...
        if(!empty($_POST)){
            ...
            if(!empty($_POST['equipment_id'])){
                ...
                if ($scanNumber) {
                    ...
                    if($auth->getRoles() == 'technician'){
                        if($this->app_setting()->must_approved == 1){
                            $approved = 1;
                        } else {
                            $approved = 0;
                        }
                    ...
                    if($result){
                        if(!empty($company->mail)){
                            $this->sendMail($lastInserId);
                        }
    ...
    public function edit(int $id){
        $intervention = $this->Intervention->findWithUser((int) $id);
        if($intervention === false) {
            $this->session->setFlash('danger', "Cette intervention n'existe pas !");
            return $this->redirect('/admin/interventions');
        }
        // Vérifie si créateur et/ou super admin
        if($this->session->read('auth')->id != $intervention->auser_id) {
            if($this->session->read('auth')->roles != 'super admin'){
                return $this->redirect('/i/' . $intervention->number_link);
            }
        }
        $client = $this->Client->find('id', $intervention->client_id);
        $equipment = $this->Equipment->find('id', $intervention->equipment_id);
        ...
        if (!empty($_POST)) {
            ...
            if (isset($_POST['received_date'])) {
                ...
                if($result){
                    $this->session->setFlash('success', "Date de dépôt mise à jour !");
                    $this->redirect('/admin/edit-intervention/'.$id);
                }
            }
            // Send BAO file
            ...
            if (isset($_POST['sendmail'])) {
                $this->sendMailUpdate($intervention->i_id);
                $this->session->setFlash('success', "Le courriel a été envoyé à " . $client->mail);
                return $this->home();
            }

            // Permet d'ajouter la date de retrait (remise du matériel au client)
            ...
            if (isset($_POST['approve_inter'])) {
                ...
                if($result){
                    $this->session->setFlash('success', "L'intervention a bien été approuvée !");
                    return $this->home();
                }
            }
        ...
        $status = $this->Status->extract('id', 'title');
        $users = $this->User->extract('id', 'pseudo');
        ...
        $this->breadcrumbs->addCrumb('Accueil', '/admin')
            ...
            ->addCrumb($client->fullname, '/admin/client/'.$client->id)
            ->addCrumb($equipment->name,'/admin/edit-equipment/'.$equipment->id)
            ->addCrumb($intervention->title, '/admin/intervention/'.$intervention->i_id);
        $this->breadcrumbs->render();
    ...
    public function showHtml(int $id)
    {
        ...
        $tasks = $this->InterventionTask->find('id', $intervention->id);
        $client = $this->Client->find('id', $intervention->client_id);
        ...
        if(!empty($_POST)){
            ...
            // Retourne null si checkbox non cochée
            $data['reinstall_os'] = $data['reinstall_os'] ?? null;
            $data['consumption'] = $data['consumption'] ?? null;
            $data['memtest'] = $data['memtest'] ?? null;
            $data['fast_format'] = $data['fast_format'] ?? null;
            $data['hard_format'] = $data['hard_format'] ?? null;
            $data['chkdsk'] = $data['chkdsk'] ?? null;
            $data['restor_os'] = $data['restor_os'] ?? null;
            $data['reinitial_os'] = $data['reinitial_os'] ?? null;
            $data['check_os'] = $data['check_os'] ?? null;
            $data['reinstall_os'] = $data['reinstall_os'] ?? null;
            $data['updates_os'] = $data['updates_os'] ?? null;
            $data['clean_tower'] = $data['clean_tower'] ?? null;
            $data['clean_os'] = $data['clean_os'] ?? null;
            $data['install_netbox'] = $data['install_netbox'] ?? null;
            // Si tout est OK, on envoi la requete
            if(!$this->InterventionTask->find('id', $intervention->id)){
                $result = $this->InterventionTask->create($data);
            } else {
                $tasks = $this->InterventionTask->find('id', $intervention->id);
                $result = $this->InterventionTask->update('id', (int) $tasks->id,$data);
            }
            if($result){
                $this->session->setFlash('success', "Cases à cocher enregistrées.");
    ...
    public function search()
    {
        ...
        $search_date = '';
        $interventions = '';
        if(isset($_GET['by_keyword'])){
            if(!empty($_GET['search']['keyword'])) {
                $search_keyword = $_GET['search']['keyword'];
                $interventions = $this->Intervention->searchWithAllData($search_keyword);
            }
        }

        if(isset($_GET['by_date'])){
            if(!empty($_GET['year']) && !empty($_GET['month'])) {
                $search_date = $_GET['month'] . '/' .$_GET['year'];
                $interventions = $this->Intervention->searchByDateWithAllData($_GET['year'], $_GET['month']);
            }
        }
    ...
    private function sendMailUpdate(int $id)
    {
        ...
        if ($viewJson->mail_service == 'mailjet'){
            ...
            $sendMail->sendDataSmtpMailer('Votre suivi intervention', $client->mail, $company->mail, $company->cname, [
                'intervention' => $intervention,
                'client' => $client,
                'user' => $user,
                'company' => $company,
    ...
    private function sendMail(int $id)
    {
        ...
        if ($viewJson->mail_service == 'mailjet'){
            // Envoi le mail avec le service externe Mailjet
            $sendMail->sendDataMailJet("Confirmation d'intervention", $client->mail, $client->fullname, [
                'intervention' => $intervention,
                'client' => $client,
                'company' => $company,
                'equipment' => $equipment,
                'categoryEquipment' => $categoryEquipment,
                'viewJsonSetting' => $viewJsonSetting
            ], 'templates/mail_newintervention.twig');
        } elseif ($viewJson->mail_service == 'swiftmailer'){
            // Envoi le mail avec Swiftmailer
            $sendMail->sendDataSmtpMailer("Confirmation d'intervention", $client->mail, $company->mail, $company->cname, [
                'intervention' => $intervention,
                'client' => $client,
                'company' => $company,
                'equipment' => $equipment,
