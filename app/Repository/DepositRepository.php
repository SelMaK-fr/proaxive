...
class DepositRepository extends BaseRepository
{
    ...
    public function joinForId(int $id)
    {
        return $this->makeQueryDefault()
            ->select(null)
            ->select('deposit.*, i.ref_number i_reference, i.ref_for_link, i.package_price, cy.name cy_name, cy.address cy_address, cy.phone cy_phone, cy.mail cy_mail, 
            cy.zipcode cy_zipcode, cy.city cy_city, cy.siret, cy.ape, cy.rc_pro, cy.type cy_type, cy.created_at cy_created_at, e.name e_name, e.id e_id, e.type_name e_type_name, e.e_serial e_serial, e.created_at e_created_at, c.city c_city, c.zipcode c_zipcode, c.department c_department, c.phone c_phone,
