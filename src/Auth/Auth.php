...
class Auth
{
    ...
    public function getRoles()
    {
        if($this->logged()){
            //return $_SESSION['auth']->rid;
            return $this->session->read('auth')->roles;
        }
    }
    ...
    public function login($db, $pseudo, $password, $remember = false){
        $user = $db->prepare('SELECT * FROM pl15x_ausers WHERE pseudo = ?', [$pseudo], null, true);
    ...
    public function resetPassword($db, $user_mail, $id){
        ...
        $viewJsonSetting = json_decode($viewJsonFile, true);
        ...
        if(isset($user_mail)){
            ...
            if ($viewJson->mail_service == 'mailjet'){
                ...
                $sendMail->sendDataSmtpMailer('Réinitialisation du mot de passe', $user_mail, $user_mail, 'Proaxive Lite', [
                    'token' => $token,
                    'user_mail' => $user_mail,
                    'id' => $id,
                    'viewJson' => $viewJsonSetting
                ], '/templates/mail_initpassword.twig');
            }
