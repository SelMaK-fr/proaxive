...
class DepositRepository extends BaseRepository
{
    ...
    public function joinForId(int $id)
    {
        return $this->makeQueryDefault()
            ...
            cy.zipcode cy_zipcode, cy.city cy_city, cy.siret, cy.ape, cy.rc_pro, cy.type cy_type, cy.created_at cy_created_at, e.name e_name, e.id e_id, e.type_name e_type_name, e.e_serial e_serial, e.brand_name e_brand_name, e.created_at e_created_at, u_password, c.city c_city, c.zipcode c_zipcode, c.department c_department, c.phone c_phone,
            c.mobile c_mobile, c.phone c_phone, c.favorite_contact c_favorite_contact, c.address c_address, c.mail c_mail, c.login_id,
